...
use_compile = False #torch.compile interfers with HelloSwag eval and Generation.
if use_compile:
...
#create the log directory we will wirte the checkpoints to  and log to 
log_dir = "log"
...
for i in range(max_steps):
    ...
    if master_process:
        print(f"step {i}, loss : {loss_accum.item()},norm : {norm:.4f}, dt : {dt*1000:.2f}ms, tok/sec :{tokesn_per_sec}")
        with open(log_file, "a") as f:
